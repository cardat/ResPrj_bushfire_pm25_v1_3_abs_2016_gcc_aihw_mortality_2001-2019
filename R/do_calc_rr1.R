...
do_calc_rr1 <- function(
    mrg_dat,
    # Define the percentile. 0.90 will exclude the upper 10th percent
    threshold = 1.00
){
  ## R1 - review the number of deaths attributable to non-exceptional days
  
  # Calculate the minimum pm25_pred for each gcc_code16
  min_values <- mrg_dat[, .(pm25_min = min(pm25_pred)), by = gcc]
  
  mrg_dat <- merge(mrg_dat, min_values, by = "gcc", all.x = TRUE)
  
  # delta is the observed minus the minimum possible
  mrg_dat[, delta := pm25_pred - pm25_min]
