...
do_calc_an_all_ages1 <- function(
  ...
  for (gcc_val in gccs) {
    ...
    for (col in c("rr", "lb", "ub")) {
      set(an_all_ages_dt1, which(an_all_ages_dt1$date == merged_dt$date), 
          paste0(gcc_val, "_", col), merged_dt[[col]])
    }
  }
  return(an_all_ages_dt1)
}
